...
export async function POST(
  ...
  try {
    ...
    const { publicaId, readerUrl } = session
    ...
    const articles = await prisma.$queryRaw`
      SELECT 
        a.id, a.pricing, a."creatorId"
      FROM articles a
      WHERE a.id = ${params.id} AND a."contentType" = 'audio'
      LIMIT 1
    `
    ...
    console.error('Error updating article with publica.la ID:', error)
    return NextResponse.json({ 
