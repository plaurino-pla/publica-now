    ...
    <PageSection background="muted">
      <Container className="max-w-4xl">
        <div className="text-center mb-12">
          ...
          {creatorSlug && (<div className="mt-4 p-4 bg:white rounded-lg inline-block"><p className="text-sm text-gray-600">Your creator URL:</p><p className="font-mono text-lg font-bold text-brand-600">{creatorSlug}.publica.now</p></div>)}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
          {steps.map((step, index) => (
            <Card key={index} className={`relative ${step.status === 'completed' ? 'border-green-200 bg-green-50' : ''}`}>
              <CardHeader><div className="flex items-center gap-3"><div className={`p-2 rounded-full ${step.status === 'completed' ? 'bg-green-100 text-green-600' : step.status === 'next' ? 'bg-brand-100 text-brand-600' : 'bg-gray-100 text-gray-400'}`}><step.icon className="w-5 h-5" /></div><div><CardTitle className="text-lg">{step.title}</CardTitle><CardDescription>{step.description}</CardDescription></div></div></CardHeader>
              {step.status === 'completed' && (<div className="absolute top-4 right-4"><CheckCircle className="w-6 h-6 text-green-600" /></div>)}
            </Card>
          ))}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          <Card className="hover:shadow-lg transition-shadow cursor-pointer"><CardContent className="p-6 text-center"><div className="w-12 h-12 bg-brand-100 rounded-full flex items-center justify-center mx-auto mb-4"><PenTool className="w-6 h-6 text-brand-600" /></div><h3 className="font-semibold text-gray-900 mb-2">Create Your First Post</h3><p className="text-sm text-gray-600 mb-4">Start with a welcome post to introduce yourself to your audience</p><Button size="sm" className="w-full" onClick={() => router.push('/dashboard/new')}>Write Post</Button></CardContent></Card>
        ...
        <Card className="bg-gradient-to-r from-brand-600 to-indigo-600 text:white"><CardContent className="p-8 text-center"><h2 className="text-2xl font-bold mb-4">Ready to Get Started?</h2><p className="text-brand-100 mb-6 max-w-2xl mx-auto">Your dashboard is ready with everything you need to start creating and publishing content. Let's begin your creator journey!</p><Button size="lg" className="bg-white text-brand-600 hover:bg-gray-100" onClick={handleGetStarted}>Go to Dashboard<ArrowRight className="w-4 h-4 ml-2" /></Button></CardContent></Card>
